...
trainFile = './trainData/pems04_volume_train.mat'
testFile = './testData/pems04_volume_test.mat'
ConnectionFile= './pems04_connected_nodes.mat'
Lfile='./Lsym_PeMS04.mat'
...
L0,L1,L2,L= trainLabel.shape
totalelementnumber=L0*L1*L2*L
number = 40

for i in range(number):   #随机次数
    trainPhi = np.ones(totalelementnumber)
    missing_rate=0.1*np.random.randint(1,10)
    num_missing = round(missing_rate * totalelementnumber)  # missing rate
    # indexvex_0_1=np.ones([trainLabel.shape[0], L])
    index = np.arange(totalelementnumber, dtype=int)
    np.random.shuffle(index)
    missing_index = (index[:num_missing])

    trainPhi[missing_index] = 0
    trainPhi=np.reshape(trainPhi,trainLabel.shape)
...
repeattimes=30
Len0,Len1,Len2,Len= testLabel.shape
totalelementnumber_test=Len0*Len1*Len2*Len
for i in range(9):
    testmissing_rate=0.1*(i+1) #缺失率
    testnum_missing = round(testmissing_rate * totalelementnumber_test)  # missing rate
    for j in range(repeattimes):
        index1 = np.arange(totalelementnumber_test, dtype=int)
        testPhi = np.ones(totalelementnumber_test)
        # testindexvex_0_1 = np.ones([testLabel.shape[0], L])
        np.random.shuffle(index1)
        missing_index = (index1[:testnum_missing])

        testPhi[missing_index] = 0
        testPhi=np.reshape(testPhi,testLabel.shape)
